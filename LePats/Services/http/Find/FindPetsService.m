...
-(void)requestDisPetWithLat:(CGFloat)fLat lng:(CGFloat)fLng dis:(double)dDis
{
    ...
    NSString *strUrl = [NSString stringWithFormat:@"%@pets/user/getNearByUserPets.do?lat=%f&lng=%f&raidus=%f&isopen=%i&userid=%@&token=%@%@",LEPAT_HTTP_HOST,fLat,fLng,dDis,user.nIsOpen,user.strUserId,user.strToken,LEPAT_VERSION_INFO];
...
-(void)requestPetLocation:(CGFloat)fLat long:(CGFloat)fLng type:(int)nType
{
    ...
    NSString *strUrl = [NSString stringWithFormat:@"%@pets/user/getNearByUserPets.do?lat=%f&lng=%f&isopen=%i&userid=%@&token=%@%@",LEPAT_HTTP_HOST,
                        fLat,fLng,user.nIsOpen,user.strUserId,user.strToken,LEPAT_VERSION_INFO];
    [self sendRequest:strUrl];   
}
...
{
    UserInfo *user = [UserInfo sharedUserInfo];//pets/user/getNearByUserPets.do
    NSString *strUrl = [NSString stringWithFormat:@"%@pets/user/getNearByUserPets.do?lat=%f&lng=%f&psi=%d&isopen=%i&&userid=%@&token=%@%@",LEPAT_HTTP_HOST,fLat,fLng,nPet,user.nIsOpen,user.strUserId,user.strToken,LEPAT_VERSION_INFO];
    [self sendRequest:strUrl];
}
